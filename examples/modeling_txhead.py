...
class BertForQuestionAnswering(PreTrainedBertModel):
    ...
    def __init__(self, config):
        ...
        '''
        self.LayerNorm1 = BertLayerNorm(config.max_position_embeddings, eps=1e-12)
        self.LayerNorm2 = BertLayerNorm(config.max_position_embeddings, eps=1e-12)
        self.LayerNorm3 = BertLayerNorm(config.max_position_embeddings, eps=1e-12)
        self.LayerNorm4 = BertLayerNorm(config.max_position_embeddings, eps=1e-12)
        self.LayerNorm5 = BertLayerNorm(config.max_position_embeddings, eps=1e-12)
        self.LayerNorm6 = BertLayerNorm(config.max_position_embeddings, eps=1e-12)
        self.LayerNorm7 = BertLayerNorm(config.max_position_embeddings, eps=1e-12)
        self.LayerNorm8 = BertLayerNorm(config.max_position_embeddings, eps=1e-12)

        #convs
        self.conv512 = nn.Conv1d(in_channels=768, out_channels=512, kernel_size=3, padding=1)
        self.conv256 = nn.Conv1d(in_channels=768, out_channels=256, kernel_size=3, padding=1)
        self.conv128 = nn.Conv1d(in_channels=768, out_channels=128, kernel_size=3, padding=1)
        self.conv64 = nn.Conv1d(in_channels=768, out_channels=64, kernel_size=3, padding=1)
        self.conv32 = nn.Conv1d(in_channels=768, out_channels=32, kernel_size=3, padding=1)
        self.conv16 = nn.Conv1d(in_channels=768, out_channels=16, kernel_size=3, padding=1)
        self.conv8 = nn.Conv1d(in_channels=768, out_channels=8, kernel_size=3, padding=1)
        self.conv4 = nn.Conv1d(in_channels=768, out_channels=4, kernel_size=3, padding=1)

        self.conv_512_1= nn.Conv1d(in_channels=768, out_channels=512, kernel_size=3, padding=1) 
        self.conv_512_2= nn.Conv1d(in_channels=512, out_channels=512, kernel_size=3, padding=1)
        self.conv_256_1= nn.Conv1d(in_channels=512, out_channels=256, kernel_size=3, padding=1)
        self.conv_256_2= nn.Conv1d(in_channels=256, out_channels=256, kernel_size=3, padding=1)
        self.conv_128_1= nn.Conv1d(in_channels=256, out_channels=128, kernel_size=3, padding=1)
        self.conv_128_2= nn.Conv1d(in_channels=128, out_channels=128, kernel_size=3, padding=1)
        
        self.conv_64_1= nn.Conv1d(in_channels=128, out_channels=64, kernel_size=3, padding=1)
        self.conv_64_2= nn.Conv1d(in_channels=64, out_channels=64, kernel_size=3, padding=1)
        self.conv_32_1= nn.Conv1d(in_channels=64, out_channels=32, kernel_size=3, padding=1)
        self.conv_32_2= nn.Conv1d(in_channels=32, out_channels=32, kernel_size=3, padding=1)
        self.conv_16_1= nn.Conv1d(in_channels=32, out_channels=16, kernel_size=3, padding=1)
        self.conv_16_2= nn.Conv1d(in_channels=16, out_channels=16, kernel_size=3, padding=1)

        self.conv_8_1= nn.Conv1d(in_channels=16, out_channels=8, kernel_size=3, padding=1)
        self.conv_8_2= nn.Conv1d(in_channels=8, out_channels=8, kernel_size=3, padding=1)
        self.conv_4_1= nn.Conv1d(in_channels=8, out_channels=4, kernel_size=3, padding=1)
        self.conv_4_2= nn.Conv1d(in_channels=4, out_channels=4, kernel_size=3, padding=1)
        
        self.conv_out=nn.Conv1d(in_channels=4, out_channels=2, kernel_size=3, padding=1)
        '''
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None):
        ...
        log_out=torch.cat((out.unsqueeze(-1),logits.unsqueeze(-1)),-1)
        lo_shape=log_out.shape
        log_out=log_out.reshape(-1,lo_shape[2],lo_shape[3]).contiguous()
        log_out=log_out.permute(0,2,1).contiguous()
        log_out = self.conv_final(log_out).permute(0,2,1).contiguous()
        log_out = log_out.squeeze(-1)
        logits = log_out.reshape(lo_shape[0],lo_shape[1],lo_shape[2])
        '''
        logits = self.qa_outputs(encoded_skip[-1]) #sequence_output1d
